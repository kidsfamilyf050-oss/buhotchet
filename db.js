...
async function verifyPassword(password, stored) {
  try {
    if (!stored.includes(':')) {
      // Старый формат SHA-256
      const old = crypto.createHash('sha256').update(password + 'buhotchet_salt_2026').digest('hex');
      return old === stored;
    }
...
async function initDb() {
  ...
  const { rows: existing } = await pool.query(
    `SELECT id, password_hash FROM users WHERE login=$1`, [adminLogin]
  );
  if (!existing.length) {
    ...
    // Обновить пароль если старый формат
    if (!existing[0].password_hash.includes(':')) {
      const hash = await hashPassword(adminPass);
      await pool.query(`UPDATE users SET password_hash=$1, role='admin', status='approved', tariff='all' WHERE id=$2`,
        [hash, existing[0].id]);
      console.log('[db] Admin пароль обновлён до PBKDF2');
    }
  }
  return true;
}
...
async function loginUser(login, password) {
  ...
  if (!ok) return { ok:false, error:'wrong_credentials' };
...
async function registerUser(data) {
  ...
  await pool.query(
    `INSERT INTO users(login,password_hash,full_name,email,phone,role,status,tariff)
     VALUES($1,$2,$3,$4,$5,'user','pending','none')`,
...
async function createResetToken(login) {
  const ln = String(login||'').trim().toLowerCase();
  const { rows } = await pool.query(
    `SELECT id,full_name,email FROM users WHERE login=$1 AND status='approved'`, [ln]
  );
  if (!rows.length) return null;
  const u = rows[0];
...
async function approveUser(userId, tariff) {
  ...
  await pool.query(
    `UPDATE users SET status='approved',tariff=$1,approved_at=NOW() WHERE id=$2`, [t, userId]
  );
}
...
async function getKv(userId, key) {
  const { rows } = await pool.query(
    `SELECT value FROM kv_store WHERE user_id=$1 AND storage_key=$2`, [userId, key]
  );
...
module.exports = {
  ...
  changePassword, createResetToken,
